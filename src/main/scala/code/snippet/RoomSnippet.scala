...
class RoomListSnippet {
  def render(in : NodeSeq) = {
    ...
    room_list.flatMap { room => Seq(
      ...
      <span> [{room.id}號] </span>{room.room_name}<br/><div class="room_comment">～ {room.room_comment} ～
      [移動:{room.move_time}][行動:{room.action_time}][反應:{room.reaction_time}]
      [ {room.max_user}人用 ] </div></a>, 
      <div class="room_option">選項：{room.option_text}</div>, <br/>) }
  }
}
...
class RoomCreateSnippet extends StatefulSnippet with Logger{
  ...
  def render =
  {
    ...
    "name=unseen_resist"  #> SHtml.checkbox(true, if (_) option_list = option_list ::: List(RoomFlagEnum.UNSEEN_RESIST)) &
    "name=vampire_weaken" #> SHtml.checkbox(true, if (_) option_list = option_list ::: List(RoomFlagEnum.VAMPIRE_WEAKEN)) &
    "name=valkyrie_enhance" #> SHtml.checkbox(false, if (_) option_list = option_list ::: List(RoomFlagEnum.VALKYRIE_ENHANCE)) &
    "name=vghost_expand"  #> SHtml.checkbox(true, if (_) option_list = option_list ::: List(RoomFlagEnum.VGHOST_EXPAND)) &
    "name=emi_enhance"    #> SHtml.checkbox(true, if (_) option_list = option_list ::: List(RoomFlagEnum.EMI_ENHANCE)) &
    "name=ellen_heal"     #> SHtml.checkbox(true, if (_) option_list = option_list ::: List(RoomFlagEnum.ELLEN_HEAL)) &
