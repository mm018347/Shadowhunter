  ...
  def talk_tag(talk: Talk, userentrys: List[UserEntry], reveal_mode: Boolean): NodeSeq = {
    ...
    */
    ...
    mtype match {
      ...
      case MTypeEnum.ACTION_ARSIS              => simple_message_tag("所有翻開之獵人回復 1 點損傷(阿爾西斯)", true, "hunter-do")
      ...
      case MTypeEnum.ACTION_HUNSOULLOWER     => simple_message_tag(useractioner.handle_name.is + " 回復 4 點損傷(療癒之魂)", true, "neutral-do")
      case MTypeEnum.ACTION_JUDGMENTACK      => simple_message_tag(useractioner.handle_name.is + " 以外所有玩家損傷值設為 7 (審判)", true, "neutral-do")
      case MTypeEnum.ACTION_LION             => simple_message_tag(useractionee.handle_name.is + " 受到的傷害減少 1 點(特羅修)", true, "hunter-do")
      case MTypeEnum.ACTION_EMMA_W           => simple_message_tag("所有翻開之獵人回復 1 點損傷(艾瑪)", true, "hunter-do")
